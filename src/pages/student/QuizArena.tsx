...
const QuizArena = () => {
  // ── Real auth — no more hardcoded user ──────────────────────────────────────
  ...
  const nextQuestion = async () => {
    ...
    // Check if current answer was correct to get the true final score
    const currentQuestion = currentQuiz?.questions[currentQuestionIndex] as Question;
    const lastAnswerCorrect = selectedAnswer === currentQuestion?.correctAnswer;

    // score state hasn't updated yet for this answer (React batches state),
    // so we compute finalScore directly here
    const finalScore = score + (lastAnswerCorrect ? 0 : 0); 
    // score was already incremented in handleAnswer via setScore — 
    // we just need to read it after the last handleAnswer call settled.
    // To be safe, recalculate from scratch using the actual current value:
    const xpEarned = Math.max(
      1,
      Math.round((score / totalQuestions) * (currentQuiz?.xp_reward || 10))
    );

    setEarnedXP(xpEarned);
    setQuizCompleted(true);

    // ── Award XP — new signature: awardXP(amount, reason) ────────────────
    if (user && xpEarned > 0) {
                    ...
                    {loadingAssigned ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Join a class to see teacher-assigned quizzes
                        </p>
                      </div>
                    ) : (
            ...
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <div className="flex items-center justify-center gap-4 mb-8">
                  <Badge variant="secondary" className="py-2 px-4 text-lg gap-2">
                    <Star className="w-5 h-5" />
                    +{earnedXP} XP earned
                  </Badge>
                </div>
                <Button onClick={restartQuiz} size="lg">Take Another Quiz</Button>
              </CardContent>
            </Card>
