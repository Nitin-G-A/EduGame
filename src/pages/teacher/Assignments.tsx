...
const TeacherAssignments = () => {
  ...
  const fetchSubmissions = async (assignmentId: string) => {
    ...
    try {
      ...
      setSubmissions(
        (data || []).map((sub) => ({ ...sub, profiles: profileMap.get(sub.student_id) || null }))
      );
    } catch (error: any) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) { toast.error('Please upload PDF, Word, or PowerPoint files only'); return; }
  ...
  const uploadFile = async (): Promise<string | null> => {
    ...
    setIsUploading(true);
    try {
      return await uploadFileToStorage(uploadedFile, user.id, 'assignments');
    } catch (error: any) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (uploadedFile) fileUrl = await uploadFile();
      ...
      setNewAssignment({ title: '', instructions: '', classId: '', deadline: '', maxPoints: 100 });
      setUploadedFile(null);
      if (fileInputRef.current) fileInputRef.current.value = '';
      fetchData();
    } catch (error: any) {
  ...
  const handleGradeWithAI = async (submission: Submission) => {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        .from('assignment_submissions')
        .update({ grade: data.grade, feedback: data.feedback, status: 'graded', graded_at: new Date().toISOString() })
        .eq('id', submission.id);
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-4">
              <div>
                <Label>Title</Label>
                <Input placeholder="e.g., Week 3 Problem Set" value={newAssignment.title}
                  onChange={(e) => setNewAssignment({ ...newAssignment, title: e.target.value })} />
              </div>
              <div>
                <Label>Instructions</Label>
                <Textarea placeholder="Describe what students need to do..." value={newAssignment.instructions}
              ...
              <div>
                ...
                <input ref={fileInputRef} type="file" accept=".pdf,.doc,.docx,.ppt,.pptx"
                  onChange={handleFileChange} className="hidden" id="assignment-file" />
                {!uploadedFile ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input type="number" value={newAssignment.maxPoints}
      ...
      {classes.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground">You need to create a class before you can create assignments.</p>
          </CardContent>
        </Card>
                  ...
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="secondary">{assignment.submission_count} submissions</Badge>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <p className="text-sm text-muted-foreground mb-4 line-clamp-2">{assignment.instructions}</p>
                    <div className="flex items-center justify-between">
                      ...
                      {assignment.file_url && <SecureFileLink filePath={assignment.file_url} variant="view" />}
                    </div>
                  </CardContent>
                </Card>
              ))}
          ...
          <TabsContent value="grading" className="mt-4">
            <div className="grid lg:grid-cols-3 gap-6">
              <div className="space-y-2">
                ...
                {assignments.map((assignment) => (
                  <Card key={assignment.id}
                    className={`cursor-pointer transition-colors hover:border-primary ${selectedAssignment?.id === assignment.id ? 'border-primary bg-primary/5' : ''}`}
                    onClick={() => { setSelectedAssignment(assignment); fetchSubmissions(assignment.id); }}>
                    <CardContent className="p-4">
                      <p className="font-medium text-sm">{assignment.title}</p>
                      <p className="text-xs text-muted-foreground">{assignment.submission_count} submissions</p>
                    </CardContent>
                  </Card>
                ))}
              </div>
                ...
                {!selectedAssignment ? (
                  <Card>
                    <CardContent className="py-12 text-center">
                      ...
                      <p className="text-muted-foreground">Select an assignment to view and grade submissions</p>
                    </CardContent>
                  </Card>
                ) : loadingSubmissions ? (
                    ...
                    {submissions.map((submission) => (
                      <Card key={submission.id}>
                        <CardContent className="p-4">
                          ...
                          {submission.status !== 'graded' && (
                            <Button size="sm" onClick={() => handleGradeWithAI(submission)}
                              disabled={gradingId === submission.id} className="gap-2">
