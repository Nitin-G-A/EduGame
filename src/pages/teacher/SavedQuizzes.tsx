  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const { error } = await supabase
        .from('quizzes')
        .update({ title: editingQuiz.title, difficulty: editingQuiz.difficulty, class_id: editingQuiz.class_id || null })
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold flex items-center gap-2">
          <FileQuestion className="w-8 h-8 text-primary" /> My Quizzes
        </h1>
        <p className="text-muted-foreground mt-1">View, edit, and manage your created quizzes</p>
      </div>
      ...
      {quizzes.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground">Create quizzes from the Quiz Generator page.</p>
          </CardContent>
        </Card>
      ) : (
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <CardDescription className="flex items-center gap-2 mt-1">
                      {quiz.classes?.name
                        ? <><Users className="w-3 h-3" />{quiz.classes.name}</>
              ...
              <CardContent>
                <div className="flex items-center gap-4 text-sm text-muted-foreground mb-4">
                  <div className="flex items-center gap-1">
                    <FileQuestion className="w-4 h-4" />{quiz.questions.length} questions
                  </div>
                  <div className="flex items-center gap-1">
                    <Clock className="w-4 h-4" />{format(new Date(quiz.created_at), 'PP')}
                  </div>
                </div>
                            ...
                            <CardContent className="p-4">
                              ...
                              <div className="space-y-2">
                                {q.options.map((option: string, optIndex: number) => (
                                  <div key={optIndex}
                                    className={`p-2 rounded text-sm ${optIndex === q.correctAnswer ? 'bg-primary/10 text-primary font-medium' : 'bg-muted'}`}>
                      ...
                      {editingQuiz && (
                        <div className="space-y-4 mt-4">
                          ...
                          <div>
                            ...
                            <Select value={editingQuiz.difficulty}
                          ...
                          <div>
                            ...
                            <Select value={editingQuiz.class_id || 'none'}
                              onValueChange={(v) => setEditingQuiz({ ...editingQuiz, class_id: v === 'none' ? null : v })}>
