...
const LessonPlanner = () => {
  ...
  const [config, setConfig] = useState({ unitName: '', duration: '', grade: '' });

  const handleGenerate = async () => {
    if (!config.unitName.trim()) { toast.error('Please enter a unit/topic name'); return; }
    if (!config.duration.trim()) { toast.error('Please enter the duration'); return; }
    if (!config.grade.trim()) { toast.error('Please enter the grade/class level'); return; }

    setIsGenerating(true);
    try {
      const response = await supabase.functions.invoke('ai-lesson-planner', {
        body: { unitName: config.unitName, duration: config.duration, grade: config.grade },
      });
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold flex items-center gap-2">
          <Lightbulb className="w-8 h-8 text-primary" /> AI Lesson Planner
        </h1>
        <p className="text-muted-foreground mt-1">Generate comprehensive lesson plans in seconds using AI</p>
      </div>

      <div className="grid lg:grid-cols-3 gap-6">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Sparkles className="w-5 h-5" /> Configuration</CardTitle>
            <CardDescription>Set your lesson parameters</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <Label>Unit/Topic Name</Label>
              <Input placeholder="e.g., Introduction to Recursion" value={config.unitName}
                onChange={(e) => setConfig({ ...config, unitName: e.target.value })} />
            </div>
            <div>
              <Label>Duration (weeks/days)</Label>
              <Input placeholder="e.g., 2 weeks, 5 days" value={config.duration}
                onChange={(e) => setConfig({ ...config, duration: e.target.value })} />
            </div>
            <div>
              <Label>Grade/Class Level</Label>
              <Input placeholder="e.g., 10th std, PUC, BTech 2nd year" value={config.grade}
          ...
          {!lessonPlan ? (
              ...
              <CardContent className="h-full flex items-center justify-center py-12">
                <div className="text-center">
                  ...
                  <p className="text-muted-foreground max-w-sm">
                    Configure your lesson parameters and let AI generate a comprehensive plan.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <>
              ...
              <Card>
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center gap-2 text-lg">
                    <Target className="w-5 h-5 text-primary" /> Learning Objectives
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <ul className="space-y-2">
                    {lessonPlan.objectives.map((obj, index) => (
                      <li key={index} className="flex items-start gap-2 text-sm">
                        <Badge variant="outline" className="mt-0.5">{index + 1}</Badge>{obj}
                      </li>
                    ))}
                  </ul>
                </CardContent>
              ...
              <Card>
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center gap-2 text-lg">
                    <BookOpen className="w-5 h-5 text-primary" /> Teaching Activities
                  </CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center gap-2 text-lg">
                    <AlertTriangle className="w-5 h-5 text-destructive" /> Common Student Mistakes
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <ul className="space-y-2">
                    {lessonPlan.commonMistakes.map((mistake, index) => (
                      <li key={index} className="flex items-start gap-2 text-sm">
                        <span className="text-destructive">•</span>{mistake}
                      </li>
                    ))}
                  </ul>
                </CardContent>
              ...
              <Card>
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center gap-2 text-lg">
                    <HelpCircle className="w-5 h-5 text-primary" /> Quick Quiz Questions
                  </CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center gap-2 text-lg">
                    <Clock className="w-5 h-5 text-primary" /> Suggested Class Flow
                  </CardTitle>
                </CardHeader>
                <CardContent>
