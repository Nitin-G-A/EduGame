...
const TeacherClasses = () => {
  ...
  const handleCreate = async () => {
    if (!newClass.name.trim()) { toast.error('Please enter a class name'); return; }
    if (!user?.id) { toast.error("User not authenticated"); return; }
    setIsCreating(true);
    try {
      // ── join_code is auto-generated by the DB trigger set_class_join_code ──
     const { error } = await supabase.from('classes').insert([{
       name: newClass.name,
       description: newClass.description || null,
       subject: 'computer_science',
       teacher_id: user.id,
       join_code: 'TEMP', // Required by TypeScript/Schema, trigger will overwrite in DB
     }]);
  ...
  const handleUpdate = async () => {
    ...
    if (!editForm.name.trim()) { toast.error('Please enter a class name'); return; }
    setIsUpdating(true);
    try {
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-2">
            <Users className="w-8 h-8 text-primary" /> My Classes
          </h1>
          <p className="text-muted-foreground mt-1">Manage your virtual classrooms and students</p>
        </div>
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input placeholder="e.g., Introduction to Programming" value={newClass.name}
                  onChange={(e) => setNewClass({ ...newClass, name: e.target.value })} />
              </div>
              <div>
                <Label>Description (optional)</Label>
                <Textarea placeholder="What will students learn in this class?" value={newClass.description}
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              <Label>Class Name</Label>
              <Input placeholder="e.g., Introduction to Programming" value={editForm.name}
                onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} />
            </div>
            <div>
              <Label>Description (optional)</Label>
              <Textarea placeholder="What will students learn in this class?" value={editForm.description}
            ...
            <Card key={cls.id} className="hover:border-primary/50 transition-colors">
              <CardHeader>
                <div className="flex items-start justify-between">
                  ...
                  <Badge variant="secondary" className="ml-2">{cls.member_count} students</Badge>
                </div>
              </CardHeader>
              <CardContent>
                {cls.description && (
                ...
                <div className="flex items-center justify-between p-3 rounded-lg bg-muted mb-4">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => copyJoinCode(cls.join_code)}>
                    <Copy className="w-4 h-4" />
                  </Button>
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" className="flex-1 gap-1" onClick={() => handleEdit(cls)}>
