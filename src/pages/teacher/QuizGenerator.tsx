...
const TeacherQuizGenerator = () => {
  ...
  const [quizConfig, setQuizConfig] = useState({ topic: '', difficulty: 'medium', numQuestions: 5, classId: '' });
...
const handleGenerate = async () => {
  ...
  const addCustomQuestion = () => {
    setCustomQuestions([...customQuestions, { question: '', options: ['', '', '', ''], correctAnswer: 0, explanation: '' }]);
  };
  ...
  const removeCustomQuestion = (index: number) => setCustomQuestions(customQuestions.filter((_, i) => i !== index));

  const handleSaveCustom = async () => {
    if (!customQuizTitle.trim()) { toast.error('Please enter a quiz title'); return; }
    if (customQuestions.length === 0) { toast.error('Please add at least one question'); return; }
    if (customQuestions.some(q => !q.question.trim() || q.options.some(o => !o.trim()))) {
      toast.error('Please fill in all questions and options'); return;
    }
    if (!user) return;
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold flex items-center gap-2">
          <FileQuestion className="w-8 h-8 text-primary" /> Quiz Generator
        </h1>
        <p className="text-muted-foreground mt-1">Generate quizzes using AI or create custom quizzes manually</p>
      </div>

      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList>
          <TabsTrigger value="ai" className="gap-2"><Sparkles className="w-4 h-4" /> AI Generated</TabsTrigger>
          <TabsTrigger value="custom" className="gap-2"><PenLine className="w-4 h-4" /> Custom Quiz</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="ai" className="mt-6">
          <div className="grid lg:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><Sparkles className="w-5 h-5" /> Quiz Configuration</CardTitle>
                <CardDescription>Configure your quiz settings and let AI generate the questions</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div>
                  <Label>Topic</Label>
                  <Input placeholder="e.g., Binary Search Trees, World War II" value={quizConfig.topic}
                    onChange={(e) => setQuizConfig({ ...quizConfig, topic: e.target.value })} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label>Difficulty</Label>
                    <Select value={quizConfig.difficulty} onValueChange={(v) => setQuizConfig({ ...quizConfig, difficulty: v })}>
                  ...
                  <div>
                    ...
                    <Input type="number" min={1} max={20} value={quizConfig.numQuestions}
                      onChange={(e) => setQuizConfig({ ...quizConfig, numQuestions: parseInt(e.target.value) || 5 })} />
                  </div>
                </div>
                <div>
                  <Label>Assign to Class (optional)</Label>
                  <Select value={quizConfig.classId} onValueChange={(v) => setQuizConfig({ ...quizConfig, classId: v })}>
                        ...
                        <div key={index} className="p-4 rounded-lg bg-muted">
                          ...
                          <div className="space-y-1">
                            {q.options.map((option, optIndex) => (
                              <p key={optIndex} className={`text-sm px-2 py-1 rounded ${optIndex === q.correctAnswer ? 'bg-primary/10 text-primary font-medium' : 'text-muted-foreground'}`}>
        ...
        <TabsContent value="custom" className="mt-6">
          <div className="space-y-6">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><PenLine className="w-5 h-5" /> Custom Quiz Settings</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid md:grid-cols-3 gap-4">
                  <div>
                    <Label>Quiz Title</Label>
                    <Input placeholder="Enter quiz title" value={customQuizTitle} onChange={(e) => setCustomQuizTitle(e.target.value)} />
                  </div>
                  <div>
                    <Label>Difficulty</Label>
              ...
              <Card key={qIndex}>
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <Textarea placeholder="Enter your question" value={q.question}
                    ...
                    {q.options.map((opt, optIndex) => (
                      <div key={optIndex}>
                        <Label className="flex items-center gap-2">
                          ...
                          {optIndex === q.correctAnswer && <Badge variant="secondary" className="text-xs">Correct</Badge>}
                        </Label>
                        <Input placeholder={`Option ${String.fromCharCode(65 + optIndex)}`} value={opt}
                  ...
                  <div>
                    ...
                    <Select value={q.correctAnswer.toString()}
                      onValueChange={(v) => updateCustomQuestion(qIndex, 'correctAnswer', parseInt(v))}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <Textarea placeholder="Explain why this is the correct answer" value={q.explanation}
