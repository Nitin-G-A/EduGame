...
const TeacherAnalytics = () => {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      if (!classes?.length) {
        setAnalytics({ totalStudents: 0, totalQuizzes: 0, totalAssignments: 0, averageQuizScore: 0, classStats: [] });
        setLoading(false);
        return;
      }
      ...
      const classStats = await Promise.all(
        classes.map(async (cls) => {
          ...
          const { count: assignmentCount } = await supabase
            ...
            .select('*', { count: 'exact', head: true })
            .eq('class_id', cls.id);
          return { name: cls.name, students: students || 0, quizzes: quizCount || 0, assignments: assignmentCount || 0 };
        })
      );
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Track your class performance and student engagement</p>
      </div>
